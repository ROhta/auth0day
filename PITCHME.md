...
```
@[2](Wordで行うことが殆ど)
@[3](条文単位で行う、草案と雛形の条文順は異なるので照らし合わせが大変)
...
| ----------------------------------- | ------------------------------- |
| 雛形を持っていたり<br/>いなかったり | 契約書の雛形を<br/>持っていない |
...
| --------------------------------------------------- | --------------------------------- |
| 雛形を持っていたり<br/>いなかったり                 | 契約書の雛形を<br/>持っていない   |
| 顧客企業作成・GVA TECH作成<br/>両方の雛形でレビュー | GVA TECH作成の<br/>雛形でレビュー |
...
- リソースAPIの個数分のトークン検証処理コード |
  - 処理がばらつく恐れ@fa[arrow-right]処理統一の必要 |
  - DRY原則 |
...
- [jwt-go側でPRも既に出ている](https://github.com/dgrijalva/jwt-go/pull/355) |
  - mergeが間に合わなかったため、<br/>この箇所を独自実装することとした |
